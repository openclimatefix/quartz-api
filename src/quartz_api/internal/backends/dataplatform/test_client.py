...
def mock_list_locations(
    ...
    req = dp.ListLocationsRequest.from_dict(req)  # Convert from dict to object for easier handling
    if req.user_oauth_id_filter != "access_user":
        return dp.ListLocationsResponse(locations=[]).to_dict()
...
def mock_get_forecast(
    ...
    req = dp.GetForecastAsTimeseriesRequest.from_dict(req)
...
def mock_get_observations(
    _: dict,
    metadata: object | None = None, # noqa
) -> dict:
...
def mock_get_latest_forecasts(
    ...
    req = dp.GetLatestForecastsRequest.from_dict(req)
...
class TestDataPlatformClient(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_site_forecast(self, client_mock: dp.DataPlatformDataServiceStub) -> None:
        ...
        for tc in testcases:
            ...
            client_mock.GetLatestForecast = Mock(side_effect=mock_get_latest_forecasts)
