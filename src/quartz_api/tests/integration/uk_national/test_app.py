...
async def test_national_forecast_metadata_true_and_false(
    ...
    response = await api_client_dataplatform.get(url)
    assert response.status_code == 200
    data = response.json()

    url = f"/v0/solar/GB/national/forecast?include_metadata=true&start_datetime_utc={now}"
    response_with_metadata = await api_client_dataplatform.get(url)
...
async def test_national_pvlive(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/national/pvlive")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
async def test_national_pvlive_day_after(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/national/pvlive?regime=day_after")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
async def test_gsp_forecast(
    ...
    """Test a sample endpoint for UK National forecast data."""
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/1/forecast")
    assert response.status_code == 200
...
async def test_gsp_forecast_no_location(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/100/forecast")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 0
...
async def test_gsp_pvlive(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/1/pvlive")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 10
...
async def test_gsp_forecast_all_cache_hit(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/forecast/all/")
    assert response.status_code == 200
    data = response.json()
    data = data["forecasts"]
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/forecast/all/?compact=true")
    assert response.status_code == 200
    data = response.json()
...
async def test_gsp_forecast_compact_false_gsp_ids(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/forecast/all/?gsp_ids=1,2,3")
    assert response.status_code == 200
    data = response.json()
    data = data["forecasts"]
    assert isinstance(data, list)
...
async def test_gsp_forecast_all_refresh(
    api_client_dataplatform_admin,
) -> None:
    """Test that the cache refresh endpoint returns 202 for an admin user."""
    response = await api_client_dataplatform_admin.post(
...
async def test_gsp_forecast_all_refresh_non_admin(
    api_client_dataplatform_non_admin,
) -> None:
    """Test that the cache refresh endpoint rejects a non-admin user."""
    response = await api_client_dataplatform_non_admin.post(
        "/v0/solar/GB/gsp/forecast/all/refresh",
    )
...
async def test_gsp_pvlive_all(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/pvlive/all")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 1
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/gsp/pvlive/all?gsp_ids=1,2,3")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 10
...
async def test_gsp_system(
    ...
    """Test a sample endpoint for UK National forecast data."""
    response = await api_client_dataplatform.get("/v0/system/GB/gsp/")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 11
...
#     response = await api_client_dataplatform.get("/v0/solar/GB/status")
#     assert response.status_code == 200
#     data = response.json()
#     assert isinstance(data, list)
#     assert len(data) == 11
...
async def test_gsp_status_check_last_forecast_run(
    ...
    response = await api_client_dataplatform.get("/v0/solar/GB/check_last_forecast_run")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, str)
...
async def test_system_details(
    ...
    response = await api_client_dataplatform.get("/v0/system/GB/gsp/?gsp_id=0")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 1
